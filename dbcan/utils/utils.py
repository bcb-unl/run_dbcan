...
@click.option("-i", "--input", "input_path", required=True, 
              ...
              help="run_dbcan output directory or an input file, depending on the selected function.")
@click.option("-bt", "--bedtools", "bedtools_file", 
              type=click.Path(exists=False), 
              help="Gene-wise depth TSV from cal_coverage (needed for *_abund functions).")
@click.option("-g", "--gff", "gff_file", 
              type=click.Path(exists=False), 
              help="Gene annotation GFF (required for cal_coverage).")
@click.option("-1", "--r1", "r1", 
              type=click.Path(exists=False), 
              help="R1 reads (.fq/.fa[.gz]) for abundance or coverage calculations.")
@click.option("-2", "--r2", "r2", default=None, 
              type=click.Path(exists=False), 
              help="Optional R2 reads for paired-end workflows.")
@click.option("-o", "--output", "output_path", default="output", 
              show_default=True, 
              help="Output directory or file path (meaning depends on function).")
@click.option("-a", "--abundance", "abundance", 
              default="RPM", 
              type=click.Choice(["FPKM", "RPM", "TPM"]), 
              show_default=True, 
              help="Normalization preset when summarizing read counts.")
@click.option("--db-dir", "db_dir", default="db", 
              show_default=True, 
              help="dbCAN database directory for substrate-aware summaries.")
@click.option("--overlap-base-ratio", "--overlap_base_ratio", 
              default=0.2, type=float, 
              show_default=True, 
              help="Minimum fraction of a read overlapping a gene (0.0–1.0) to count toward coverage.")
@click.option("--mapping-quality", "--mapping_quality", 
              default=30, type=int, 
              show_default=True, 
              help="Minimum MAPQ threshold when parsing BAM alignments.")
@click.option("-c", "--identity", "identity", 
              default=0.98, type=float, 
              show_default=True, 
              help="Minimum alignment identity (0.0–1.0) when filtering alignments.")
@click.option("-t", "--threads", "threads", 
              default=1, type=int, 
              show_default=True, 
              help="Parallel worker count for supported functions.")
@click.option("--hifi", "hifi", is_flag=True, 
              help="Treat reads as PacBio HiFi (coverage counting tweaks).")
